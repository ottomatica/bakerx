...
exports.handler = async argv => {
    ...
    (async () => {
        ...
        else if( provider === 'vbox' )
        {
            ...
            while(! await vbox.driver.waitForBoot(name)) { Utils.timeout(200); }
            throbber.stop();
            console.log('The VM is now ready. You can run this ssh command to connect to it.');

            let sshInfo = await vbox.getSSHConfig(name);

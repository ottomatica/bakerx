...
exports.handler = async argv => {
    ...
    if (!name || !image) {
        ...
        if (await fs.exists(bakerxfilePath)) {
            const bakerxfile = yaml.safeLoad(await fs.readFile(bakerxfilePath, 'utf8'));

            name = bakerxfile.name;
            image = bakerxfile.image;
            ip = bakerxfile.ip;
            cpus = bakerxfile.cpus;
            memory = bakerxfile.memory;
            bridged = bakerxfile.bridged;
            up = bakerxfile.up;
            sync = bakerxfile.sync;
        }
    ...
    (async () => {
        ...
        if( up )
        {
            if( await fs.exists( up ) )
            {
                ...
                await conn.exec(`echo "${up}" > ~/bakerx_up.sh`);
            }
